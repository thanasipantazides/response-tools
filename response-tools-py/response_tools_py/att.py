...
ATT_PATH = os.path.join(pathlib.Path(__file__).parent, "..", "..", "response-information", "att-data")
...
def att_thermal_blanket(mid_energies, file=None):
    ...
    _f = os.path.join(ATT_PATH, "F4_Blanket_transmission.dat") if file is None else file
...
def att_uniform_al_cdte(mid_energies, file=None, position=None):
    ...
    _f = os.path.join(ATT_PATH, f"unif_att_p{position}_theoretical.csv") if file is None else file
...
def att_pixelated(mid_energies, file=None, values=None):
    ...
    _f = os.path.join(ATT_PATH, "20240607_fosxi4_transmission.csv") if file is None else file
    att = pandas.read_csv(_f)
...
def att_al_mylar(mid_energies, file=None):
    ...
    _f = os.path.join(ATT_PATH, "thin_mylar_p3_p5_theoretical.csv") if file is None else file
    att = pandas.read_csv(_f)
    att_es, att_values = att["energy[keV]"] << u.keV, att["transmission"] << u.dimensionless_unscaled
    return np.interp(mid_energies.value, att_es.value, att_values.value, left=0, right=1) << u.dimensionless_unscaled
